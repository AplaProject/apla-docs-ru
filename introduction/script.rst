...
Контракты Apla пишутся на оригинальном (разработанном командой Apla) тьюринг-полном скриптовом языке Simvolio с компиляцией в байт-код. Язык содержит необходимый набор функций, операторов и конструкций  для реализации алгоритмов обработки данных и операций со значениями базы данных.
...
Контракт состоит из трех секций:

1. data - используется для описания передаваемых контракту данных (имена переменных и их типы);
2. conditions - в секции реализуется проверка входных данных с выводом сообщений об ошибках с помощью команд: error, warning, info; все эти команды генерируют ошибку, останавливающую работу контракта, но выводят в интерфейсе различные сообщения: критическая ошибка, предупреждение, и информативная ошибка; 
3. action - содержит основной программный код контракта выполняющий получение дополнительных данных и запись результата в таблицы базы данных.
...
- задание прав на редактирование контракта (традиционно через указание имени контракта с определенными в нем правами в специальной функции ContractConditions или прямым указанием условий доступа в поле Change conditions)
