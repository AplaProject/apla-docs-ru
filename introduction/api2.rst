...
* *data* - json со следующими полями:
        ...
        * *[payover]* - для непривязанных к аккаунту контрактов можно указать надбавку за срочность - сколько добавить к fuel_rate при вычислении оплаты,
        * передаваемые контракту параметры,
        * *signatures* - массив hex подписей значении *forsign*, которое получено из prepareMulti,
        * *time* - время, возвращенное prepare,
        * *[pubkey]* - hex публичный ключ подписавшего контракт; следует заметить, что если публичный ключ уже хранится в таблице keys данной экосистемы, то его можно не передавать,
...
prepareMulti/{name}
==============================
**POST**/ Отправляет запрос на получение строк для подписи указанного контракта выполненного несколько раз с различными параметрами. В качестве **{name}** необходимо указать имя транзакции для которой следует возвратить строку для подписи. В параметре forsign возвращается строка, которую необходимо будет подписать. Также возвращается параметры request_id и time, которые нужно будет передать вместе с подписью.
...
* *data* - json, содержащий следующие поля:
        ...
        * *[payover]* - для непривязанных контрактов можно указать надбавку за срочность - сколько добавить к fuel_rate при вычислении оплаты,
        * *[vde]* - укажите *true*, если вы будете вызывать смарт-контракт из VDE, в противном случае, этот параметр указывать не нужно.
        * *params* - массив словарей, каждый из которых является вызовом контракта name с параметрами этого словаря.Все значения должные быть строками.

.. code:: 
    
    POST
    /api/v2/prepareMulti/mycontract
...
.. code::
    ...
    {"token_ecosystem": "",
     ...
     "payover": "",
     "signed_by": "",
     "params": [{"Param1": "Value1", "Param2": "Value2"}, {"Param11": "Value11", "Param21": "Value21"}]
