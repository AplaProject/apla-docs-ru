...
Черновик команд API v2 для работы с Apla. Для работы с REST API v2 необходимо скомпилировать сервер из ветки **gorm_arch**. Графического интерфейса для API v2 в браузере нет.
...
* *[limit]* - количество записей. По умолчанию, 25.
* *[offset]* - смещение начала записей. По умолчанию, 0.
* *[vde]* - укажите *true*, если необходимо получить списко контрактов из VDE. В противном случае, этот параметр указывать не нужно.
...
* *list* - массив, каждый элемент которого содержит следующие параметры.
  ...
  * *wallet_id* - кошелек, владелец контракта. 
...
* *walletid* - идентификатор владельца контракта.
