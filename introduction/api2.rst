...
prepareMultiple/{name}
==============================
**POST**/ Отправляет запрос на получение строк для подписи указанного контракта выполненного несколько раз с различными параметрами. В качестве **{name}** необходимо указать имя транзакции для которой следует возвратить строки для подписи. В параметре forsign возвращаются строки, которые необходимо будет подписать. Также возвращается параметры request_id и time, которые нужно будет передать вместе с подписью.
...
.. code:: 
    ...
    POST
    /api/v2/prepareMultiple/mycontract
