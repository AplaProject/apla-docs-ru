...
Черновик команд API v2 для работы с EGAAS.
...
Для аутентификации используется **JWT токен** http://www.jwt.org. После получения JWT токена необхоимо передавать его при каждом запросе в заголовке в **Authorization**.

getuid
==============================
**GET** Возвращает уникальное значение, которое нужно подписать своим приватным ключом и отправить обратно серверу с помощью команды **login**. На данный момент создается временный JWT токен, который можно получить в заголовке ответа **Authorization** и в дальнейшем  передать в **Authorization** при вызове **login**. Можно сделать передачу токена в самомом JSON ответе. Все поля JSON ответа:
...
login
==============================
**POST** Аутентификация пользователя. Предварительно необходимо вызвать команду **getuid** для получения уникального значения и подписать его. В заголовке нужно передать временный JWT токен полученный вместе с getuid. В случае успешного завершения нужно получить из заголовка JWT токен и в дальнейшем передавать его во всех запросах в заголовке *Authoprization*.
