...
* **gap_between_blocks** - множитель, на который умножается количество секунд, которые нода ждет, пока не получает право сгенерить следующий блок.
* **rb_blocks_1** - максимальное количество блоков, на которое можно откатиться. Влияет на количество блоков которое можно за раз скачать.
* **new_version_url** - хост для проверки доступности новых версий(апдейт сервера).
* **number_of_nodes** - максимальное количество полных нод.
* **max_block_size** - максимальный размер блока в байтах.
* **max_tx_size** - максимальный размер транзакции в байтах.
* **max_tx_count** - максимальное количество транзакции в блоке.
* **max_columns** - максимальное количество колонок в созданном пользователем реестре.
* **max_indexes** - максимальное количество индексов в созданном пользователем реестре.
* **max_block_user_tx** - максимальное количество транзакции в блоке от одного пользователя.
* **max_fuel_tx** - максимальный расход топлива на одно транзакцию.
* **max_fuel_block** - максимальный расход топлива на блок.
* **size_fuel** - множитель, на который умножается плата за количество данных в смарт контракте.
* **commission_wallet** - адрес кошелька, на который начисляется комиссия.
* **commission_size** - процент коммиссии, который будет начисляется с каждой операции на commission_wallet.
* **fuel_rate** - курс конверсии APL к топливу.
* **full_nodes** - список нод, которые могут генерить блоки. Представляет собой список списков вида [[хост, адрес кошелька, публичный ключ],].
* **extend_cost_(funcname)** - стоимость вызова встроенной функции в fuel.
* **(table|column|page|menu|contract)_price** - стоимость создания какой либо сущности в fuel.
