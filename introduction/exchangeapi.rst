...
Для того, чтобы работать с данным API, при запуске Apla необходимо указывать следующие параметры.
