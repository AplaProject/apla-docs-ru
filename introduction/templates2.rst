...
Вызов контрактов Protypo происходит при клике на кнопке  формы (функция *Button*). При эом производится передача в контракт данных, введенных пользователем в поля формы (если имена полей формы и имена переменных в секции data вызываемого контракта совпадают, то данные передаются автоматически). В функции Button возможен вызова модального окна для подтверждения пользователем запуска контракта (Alert), а так же инициация перехода на указанную страницу после успешного выполнения контракта с передачей странице указанных параметров.
