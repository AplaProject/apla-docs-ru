...
* *Name* - имя параметра,
* *Index* - порядковый номер элемента параметра (начиная c 1) в случае, если значение параметра представлено списоком через запятую, например, ``type = full,light``, тогда ``AppParam(1, type, 2)`` возвратит *light*,  
