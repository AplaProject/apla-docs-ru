...
==========================
возвращает данные из таблицы базы данных и создает элемент **dbfind**. В *attr* возвращаются три массива - *columns* c именами колонок, *types*, где для обычной колонки указан *text*, а для Custom колонок указан тип *tags* и массив *data* с записями. Последовательность в именах колонок соответствует последовательности значений в записях в *data*.

* *Name* - имя таблицы.
* *Source* - имя источника данных с произвольным именем, которое потом следует указывать в других командах (например, *Table*) как источник данных.
...
* **Custom** - позволяет определять вычисляемые столбцы для данных, например, шаблон для кнопок и дополнительного оформления; можно определить несколько таких вычисляемых столбцов; Как правило, такие поля определяются для вывода в *Table* и других командах, которые используют полученные данные.

  * *Column* - имя колонки (любое уникальное имя),
  * *Body* - шаблон, в котром можно получать значения из других колонок в данной записи с помощью **#columnname#**.
...
Include(Name)
==========================
Команда вставляет шаблон блока с именем *Name*.
