...
* **Columns** - список возвращаемых колонок; если не указано, то возвратятся все колонки. Если имеются колонки типа JSON, то вы можете использовать обращение к полям записи с помощью записи вида **имяколонки->имяполя**. В этом случае, имя результирующей колонки будет **имяколонки.имяполя**.
* **Where** - условие поиска данных, например, ``.Where(name = '#myval#')``. Если имеются колонки типа JSON, то вы можете использовать обращение к полям записи с помощью записи вида **имяколонки->имяполя**.
...
GetBlockHistory(Source, Id)  
------------------------------
Создает элемент **getblockhistory**, заполняет его данными c историей изменений указанной записи в таблице **_blocks**, и помещает его в конструкцию *Source*, которая потом указывается в *Table* и других командах, получающих *Source* в качестве входных данных. Результирующий список отсортирован от последних изменений к более ранним.

* *Source* - произвольное имя источника данных,
* *Id* - идентификатор записи.

.. code:: js
	GetBlockHistory(BlockHistory, 1)

GetContractHistory(Source, Id)  
------------------------------
Создает элемент **getcontracthistory**, заполняет его данными c историей изменений указанной записи в таблице **_contracts**, и помещает его в конструкцию *Source*, которая потом указывается в *Table* и других командах, получающих *Source* в качестве входных данных. Результирующий список отсортирован от последних изменений к более ранним.

* *Source* - произвольное имя источника данных,
* *Id* - идентификатор записи.

.. code:: js
	GetContractHistory(ContractHistory, 1)
    
GetMenuHistory(Source, Id)  
------------------------------
Создает элемент **getmenuhistory**, заполняет его данными c историей изменений указанной записи в таблице **_menu**, и помещает его в конструкцию *Source*, которая потом указывается в *Table* и других командах, получающих *Source* в качестве входных данных. Результирующий список отсортирован от последних изменений к более ранним.

* *Source* - произвольное имя источника данных,
* *Id* - идентификатор записи.

.. code:: js
	GetMenuHistory(MenuHistory, 1)

GetPageHistory(Source, Id)  
------------------------------
Создает элемент **getpagehistory**, заполняет его данными c историей изменений указанной записи в таблице **_pages**, и помещает его в конструкцию *Source*, которая потом указывается в *Table* и других командах, получающих *Source* в качестве входных данных. Результирующий список отсортирован от последних изменений к более ранним.

* *Source* - произвольное имя источника данных,
* *Id* - идентификатор записи.

.. code:: js
	GetPageHistory(PageHistory, 1)
