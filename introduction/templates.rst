...
**BtnContract(contract, text, message, params, [class], [onsuccess], [page], [pageparams])** - создает кнопку, при нажатии на которую откроется модальное окно с сообщением для подтверждения или отказа от транзакции. Если пользователь даст согласие, то произойдет отправка контракта *contract* с параметрами, указанными в переменной *params*. В параметре *text* передается текст кнопки, а в message указывается вопрос, который будет отображаться во всплывающем окне. Четвертым параметром можно указать классы для кнопки. Вы можете указать страницу для перехода в случае успешной отправки транзакции. *onsuccess* - может быть **page** или **template**, *page* - имя страницы, *pageparams* - параметры для страницы. Например, *BtnContract(..., page, contracts, "global:1, param:'Param'")*

Эта функция может быть полезной при привязке какого-нибудь контракта к каждой записи в таблице. 

.. code:: js

      Table {
         ...
         Columns: [..., [Edit,BtnContract(MyContract,Edit,Хотите удалить элемент?, "RequestId: #id#, Name: '#name#'")]]
      }
