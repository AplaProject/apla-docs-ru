...
**And(param, [param,...])** - возвращет результат выполнения логической операции **И** со всеми перечисленными в скобках через запятую параметрами. Значение параметра принимается как *false*, если он равен пустой строке (""), 0 или false. Во всех остальных случаях значение параметра считается *true*.
...
* *page* - имя страницы;
* *params* - параметры вызова страницы из истории.
...
**BtnContract(contract, name, message, params, [class], [onsuccess], [pageparams])** - создает на странице кнопку, при клике на которой открывается модальное окно с предложением подтвердить или отказаться от вызова контракта. 
...
* *onsuccess* - имя страницы, на которую следует сделать переход в случае успешного выпонения контракта;
...
**BtnEdit( page, id )** - создает кнопку в форме шестеренки с ссылкой на указанную страницу *page* и передает *id* в качестве параметра; применяется в экранных таблицах для ссылки на редактирования элементов.

**BtnSys(page,name,[params],[class])** - создает кнопку со ссылкой на систеную страницу *page*
