...
**BtnContract(contract, text, message, params, [class])** - создает кнопку, при нажатии на которую откроется модальное окно с сообщением для подтверждения или отказа от транзакции. Если пользователь даст согласие, то произойдет отправка контракта *contract* с параметрами, указанными в переменной *params*. В параметре *text* передается текст кнопки, а в message указывается вопрос, который будет отображаться во всплывающем окне. Четвертым параметром можно указать классы для кнопки. Эта функция может быть полезной при привязке какого-нибудь контракта к каждой записи в таблице.
