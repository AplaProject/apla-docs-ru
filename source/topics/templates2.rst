...
* *Icon* - иконка,
* *Obs* - параметр, определяющий переход на виртуальную выделеную экосистему; если ``Obs: true``, то ссылка ведёт в OBS, если ``Obs: false``, то в блокчейн, если параметр не указан, то решается в зависимости от того, где было загружено меню.
